<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Amamul%20Ahasan&fontSize=50&fontColor=fff&animation=twinkling&fontAlignY=35&desc=Full%20Stack%20Web%20Developer&descAlignY=55&descSize=20"/>

<p align="center">
  <a href="https://github.com/Ahasan39">
    <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&duration=3000&pause=1000&color=58A6FF&center=true&vCenter=true&random=false&width=520&height=45&lines=Full+Stack+Web+Developer+%F0%9F%9A%80;React+%7C+Node.js+%7C+Python+%F0%9F%92%BB;Open+Source+Contributor+%F0%9F%8C%8D;Always+Learning+%26+Building+%E2%9C%A8" alt="Typing SVG" />
  </a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=Ahasan39&label=Profile%20Views&color=58A6FF&style=flat-square" alt="Profile views" />
  <img src="https://img.shields.io/github/followers/Ahasan39?label=Followers&style=flat-square&color=58A6FF" alt="Followers" />
  <img src="https://img.shields.io/github/stars/Ahasan39?label=Stars&style=flat-square&color=58A6FF&affiliations=OWNER" alt="Stars" />
</p>

---

## 👨‍💻 About Me

A highly motivated and passionate **Computer Science & Engineering** graduate from **Premier University, Chittagong** — with strong technical knowledge, problem-solving ability, and creativity. Eager to contribute to the growth of a dynamic organization while continuously developing both professional and personal skills.

- 🎓 B.Sc. in **Computer Science & Engineering** — Premier University, CTG 
- 💼 **Full Stack Web Developer** at **Webleez Limited**, Dhaka (Jan 2026 – Present)
- 🔭 Currently building **full-stack web applications** with Laravel, React & Vue.js
- 🌱 Learning **advanced React patterns, system design & DevOps**
- 👯 Open to collaborating on **open source projects**
- 💬 Ask me about **JavaScript, React, Laravel, Vue.js, Node.js, MySQL**
- 📍 Based in **Chittagong, Bangladesh**
- 📄 Check out my **[Resume](https://ahasan39.github.io/)**
- ⚡ Fun fact: I love turning ideas into working products

---

## 🛠️ Tech Stack

**Frontend**

<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,react,tailwind,bootstrap" alt="Frontend" />
</p>

**Backend**

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,python,mongodb,mysql,firebase" alt="Backend" />
</p>

**Tools & DevOps**

<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,linux,docker,postman" alt="Tools" />
</p>

---

## 📊 GitHub Analytics

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=Ahasan39&theme=2077" alt="Stats" width="48%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=Ahasan39&theme=2077&utcOffset=6" alt="Productive Time" width="48%" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=Ahasan39&theme=2077" alt="Top Languages by Repo" width="48%" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=Ahasan39&theme=2077" alt="Top Languages by Commit" width="48%" />
</p>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=Ahasan39&theme=2077" alt="Profile Details" width="97%" />
</p>

---

## 🔥 Streak

<p align="center">
  <img src="https://streak-stats.demolab.com/?user=Ahasan39&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
</p>

---

## 🏆 GitHub Trophies

<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=Ahasan39&theme=tokyonight&no-frame=true&no-bg=true&row=2&column=4&margin-w=8&margin-h=4" alt="GitHub Trophies" />
</p>

---

## 📈 Contribution Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=Ahasan39&theme=tokyo-night&hide_border=true&area=true&radius=6" alt="Activity Graph" width="95%" />
</p>

---

## 🐍 Contribution Snake

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/Ahasan39/Ahasan39/output/github-contribution-grid-snake-dark.svg" />
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/Ahasan39/Ahasan39/output/github-contribution-grid-snake.svg" />
    <img alt="Snake animation" src="https://raw.githubusercontent.com/Ahasan39/Ahasan39/output/github-contribution-grid-snake.svg" width="95%" />
  </picture>
</p>

---

## 🗂️ Project Portfolio

### ⭐ University Repository Management System

<p align="center">
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/Blade-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/REST_API-009688?style=flat-square&logo=fastapi&logoColor=white" />
  <img src="https://img.shields.io/badge/jQuery-0769AD?style=flat-square&logo=jquery&logoColor=white" />
  <img src="https://img.shields.io/badge/Chart.js-FF6384?style=flat-square&logo=chartdotjs&logoColor=white" />
</p>

A **production-ready thesis lifecycle automation platform** developed over 1.5 years for **Premier University, Chattogram** — officially integrated with the university website and actively used in production with real-time API integration.

**🔐 Access & Assignment**
- Multi-role RBAC (Admin, Advisor, Supervisor, Co-Supervisor, Panel Member, Student)
- Area of Interest-based matching algorithm
- Rank/Priority-based matching with workload balancing
- Supervisor capacity validation

**📄 Thesis Lifecycle**
- PDF Annotation Studio (in-browser feedback)
- Meeting management & history tracking
- Excel-based bulk group import
- Public thesis repository with real-time university API integration

**📊 Monitoring & Reporting**
- Performance monitoring dashboards
- Real-time notifications
- Analytics with Chart.js
- Production-deployed & actively used (1.5 years of development & iteration)

<p align="center">
  <a href="https://github.com/Ahasan39?tab=repositories">
    <img src="https://img.shields.io/badge/SOURCE_CODE_→-181717?style=for-the-badge&logo=github&logoColor=white" alt="Source Code" />
  </a>
</p>

---

### ⭐ Smart Restaurant Management System

<p align="center">
  <img src="https://img.shields.io/badge/Laravel_12-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/Vue.js_3-4FC08D?style=flat-square&logo=vue.js&logoColor=white" />
  <img src="https://img.shields.io/badge/Inertia.js-9553E9?style=flat-square&logo=inertia&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/Pinia-FFD859?style=flat-square&logo=pinia&logoColor=black" />
</p>

A comprehensive, full-stack restaurant management system — covering everything from admin panel & POS to customer portal, KOT, inventory, multi-language support (EN / AR / BN), and real-time order notifications.

**🏢 Admin Panel**
- Real-time dashboard & analytics
- Role-based access (Admin, Manager, Chef, Waiter, Cashier)
- Menu & QR code management
- POS & KOT system
- Inventory, supplier & expense tracking

**🌐 Public Website**
- Multi-language (English, Arabic, Bengali) with RTL support for Arabic
- Online ordering & table reservation
- Shopping cart with Pinia state management
- Fully responsive (mobile / tablet / desktop)

**👤 Customer Portal**
- Personal dashboard & order history
- Active order tracking with quick reorder & cancellation
- Profile & password management
- Sales & profit/loss reporting

<p align="center">
  <a href="https://github.com/Ahasan39?tab=repositories">
    <img src="https://img.shields.io/badge/SOURCE_CODE_→-181717?style=for-the-badge&logo=github&logoColor=white" alt="Source Code" />
  </a>
</p>

---

### 🍽️ Khadyobitan — E-Commerce Platform

<p align="center">
  <img src="https://img.shields.io/badge/Laravel_11-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/React_19-61DAFB?style=flat-square&logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/Inertia.js-9553E9?style=flat-square&logo=inertia&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/Tailwind_CSS_4-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/JWT-000000?style=flat-square&logo=jsonwebtokens&logoColor=white" />
  <img src="https://img.shields.io/badge/Zustand-443E38?style=flat-square&logo=react&logoColor=white" />
  <img src="https://img.shields.io/badge/Vite_5-646CFF?style=flat-square&logo=vite&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker&logoColor=white" />
</p>

A full-stack e-commerce platform built with **Laravel 11** (backend) and **React 19 + Inertia.js** (frontend). Designed for organic & natural food stores with bilingual support (English & বাংলা).

**🛒 Storefront (React SPA)**
- 24 page components (Home, Shop, Cart, Checkout, Blog, etc.)
- Bilingual UI (EN / বাংলা) via i18next
- Product filtering, sorting & search
- Cart & wishlist with Zustand store
- Order tracking & WhatsApp integration
- 48+ Radix UI / shadcn components with Framer Motion animations

**📦 Product & Order System**
- 3-level category hierarchy with product variants, images & barcode generation
- Stock alerts & inventory tracking
- Full order lifecycle (pending → delivered) with abandoned cart tracking
- ShurjoPay, bKash & COD payments
- Pathao courier API integration

**📊 Admin & Advanced**
- 42+ admin controllers with Spatie RBAC
- Sales analytics & expense reporting
- Campaign & coupon management
- 6 events, 5 webhook listeners, 2 queue jobs, 6 model observers
- REST API v1 for mobile/external apps
- Docker configs (PHP 7.4–8.2)

<p align="center">
  <a href="https://github.com/Ahasan39/khadyobitan_newversion">
    <img src="https://img.shields.io/badge/SOURCE_CODE_→-181717?style=for-the-badge&logo=github&logoColor=white" alt="Source Code" />
  </a>
</p>

---

### 🛒 Multi Vendor E-commerce

<p align="center">
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black" />
</p>

Full-featured e-commerce platform with product management, cart system, and complete order flow.

- Product & multi-category management
- Shopping cart & secure checkout
- Admin dashboard with order control
- Customer authentication & profiles
- Variant product support

<p align="center">
  <a href="https://github.com/Ahasan39/usupershop">
    <img src="https://img.shields.io/badge/SOURCE_CODE_→-181717?style=for-the-badge&logo=github&logoColor=white" alt="Source Code" />
  </a>
</p>

---

### 🌐 Developer Portfolio

<p align="center">
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black" />
  <img src="https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=flat-square&logo=tailwindcss&logoColor=white" />
</p>

Modern personal portfolio site built to showcase projects, skills & contact info for recruiters.

- Fully responsive across devices
- Smooth scroll & animations
- Projects & skills showcase
- Contact form integration
- Semantic, accessible markup

<p align="center">
  <a href="https://github.com/Ahasan39/ahasan39.github.io">
    <img src="https://img.shields.io/badge/SOURCE_CODE_→-181717?style=for-the-badge&logo=github&logoColor=white" alt="Source Code" />
  </a>
</p>

---

### 🏥 MediBook — Doctor Appointment System

<p align="center">
  <img src="https://img.shields.io/badge/PHP-777BB4?style=flat-square&logo=php&logoColor=white" />
  <img src="https://img.shields.io/badge/Laravel-FF2D20?style=flat-square&logo=laravel&logoColor=white" />
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white" />
  <img src="https://img.shields.io/badge/REST_API-009688?style=flat-square&logo=fastapi&logoColor=white" />
</p>

RESTful API for managing doctor-patient appointments with auth, scheduling & role-based access.

- JWT-based authentication
- Doctor & patient management
- Appointment scheduling system
- Role-based access control
- Laravel resource routes & policies

<p align="center">
  <a href="https://github.com/Ahasan39/doctor-appointment-api">
    <img src="https://img.shields.io/badge/SOURCE_CODE_→-181717?style=for-the-badge&logo=github&logoColor=white" alt="Source Code" />
  </a>
  &nbsp;
  <a href="https://github.com/Ahasan39?tab=repositories">
    <img src="https://img.shields.io/badge/ALL_REPOSITORIES_→-0A66C2?style=for-the-badge&logo=github&logoColor=white" alt="All Repositories" />
  </a>
</p>

---

## 🤝 Connect with Me

<p align="center">
  <a href="https://github.com/Ahasan39" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <a href="https://linkedin.com/in/ahasan39" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="mailto:amamulahasanmizan71@gmail.com" target="_blank">
    <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" />
  </a>
  <a href="https://ahasan39.github.io/" target="_blank">
    <img src="https://img.shields.io/badge/Portfolio-FF5722?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Portfolio" />
  </a>
  <a href="tel:+8801842299275" target="_blank">
    <img src="https://img.shields.io/badge/Phone-25D366?style=for-the-badge&logo=whatsapp&logoColor=white" alt="Phone" />
  </a>
</p>

---

<p align="center">
  <img src="https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight" alt="Random Dev Quote" />
</p>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer"/>